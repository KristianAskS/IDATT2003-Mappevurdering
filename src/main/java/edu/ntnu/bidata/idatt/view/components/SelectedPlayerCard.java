...
public class SelectedPlayerCard extends HBox {
  ...
  public SelectedPlayerCard(Player player, Consumer<Player> onDelete) {
    ...
    Label shapeLbl = new Label(shape.substring(0,1).toUpperCase() + shape.substring(1).toLowerCase());
    shapeLbl.getStyleClass().add("label-listview");
    ...
    getChildren().addAll(colorBox, nameLbl, shapeLbl, spacer, deleteBtn);
  }
}